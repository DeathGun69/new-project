        ...
        <div class="row row-custome">
            ...
            <div class="col-10 content">
                    ...
                    <table class="table">
                        ...
                        <tbody>
                            ...
                            while ($raion = mysqli_fetch_array($resultRaion)) {
                                ...
                                //Запрос по унечтоженым объектам
                                $queryOD = "SELECT id_raion, objects_destroyed FROM `statistics_district` WHERE id_raion = $i";

                                $resultDestr = mysqli_query($connect, $queryOD);

                                echo "<tr>";
                                echo "<th scope='row'>" . $raion["raion_name"] . "</th>";

                                while ($destr = mysqli_fetch_array($resultDestr)) {
                                    echo "<td>" . $destr["objects_destroyed"] . "</td>";
                                }
                                $i++;
                            ...
                            <thead>
                                <tr>
                                    ...
                                    $querySum = "SELECT date, SUM(objects_destroyed) as sumDestr FROM `statistics_district` GROUP BY date";

                                    $resultSum = mysqli_query($connect, $querySum);

                                    while ($destrSum = mysqli_fetch_array($resultSum)) {
                                        echo "<th scope='col'>" . $destrSum["sumDestr"] . "</th>";
                                    }
                                    ?>
                                </tr>
                            </thead>
                ...
                <div class="table">
                    <br>
                    <div class="label">
                        <label class="prefix">Rус - интегральный территориальный риск риск уничтожения строений, объектов техносферы [объект/(пожар*год)]</label>
                    </div>
                    <br>
                    <table class="table table-sm">
                        ...
                        <tbody>
                            ...
                            while ($raion = mysqli_fetch_array($resultRaion)) {
                                ...
                                //Запрос по Rтр
                                $queryF = "SELECT ROUND(objects_destroyed/fire,5) AS ryc_dest FROM `statistics_district` WHERE id_raion = $i";
                            ...
                            <thead>
                                <tr>
                                    ...
                                    $queryR1_F = "SELECT ROUND(SUM(objects_destroyed)/(SUM(fire)), 5) AS sumRyc_destr FROM `statistics_district` GROUP BY date";

                                    $resultR1_F = mysqli_query($connect, $queryR1_F);

                                    while ($r1_fire = mysqli_fetch_array($resultR1_F)) {
                                        echo "<th scope='col'>" . $r1_fire["sumRyc_destr"] . "</th>";
                                    }
                                    ?>
                                </tr>
                            </thead>
